...
namespace RootMotion.FinalIK {
	...
	public class VRIK : IK {
		...
		public class References {
			...
			void AfterVRIK() {
				Transform leftForearmTwist = leftForearm.transform.GetChild(1);
				Transform rightForearmTwist = rightForearm.transform.GetChild(1);

				leftForearmTwist.transform.rotation = Quaternion.Slerp(leftForearmTwist.transform.rotation, leftHand.transform.rotation, 0.75f);
				rightForearmTwist.transform.rotation = Quaternion.Slerp(rightForearmTwist.transform.rotation, rightHand.transform.rotation, 0.75f); //Has no effect right now look into why
			} 
			//End my shit
